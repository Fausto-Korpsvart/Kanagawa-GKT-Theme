...
OPTIONS:
  ...
  --tweaks                Specify versions for tweaks
                          1. [dragon]	Dragon ColorSchemes version
	...
	--tweaks)
		...
		for variant in $@; do
			...
			dragon)
				dragon="true"
				ctype="-Dragon"
				echo -e "Dragon ColorScheme version! ..."
				shift
				;;
			black)
				blackness="true"
...
dragon_color() {
	sed -i "/\@import/s/color-palette-default/color-palette-dragon/" "${SRC_DIR}/sass/_tweaks-temp.scss"
	sed -i "/\$colorscheme:/s/default/dragon/" "${SRC_DIR}/sass/_tweaks-temp.scss"
}
