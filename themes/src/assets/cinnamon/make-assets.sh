...
for theme in '' '-Purple' '-Pink' '-Red' '-Orange' '-Yellow' '-Green' '-Blue' '-Grey'; do
	for type in '' '-Dragon'; do
		...
		if [[ "$type" != '' ]]; then
			...
			sed -i "s/#8ba4b0/${theme_color_dark}/g" "theme${theme}${type}"/*.svg
			sed -i "s/#4d699b/${theme_color_light}/g" "theme${theme}${type}"/*.svg
		elif [[ "$theme" != '' ]]; then
			cp -rf "theme" "theme${theme}"
			sed -i "s/#7e9cd8/${theme_color_dark}/g" "theme${theme}"/*.svg
			sed -i "s/#4d699b/${theme_color_light}/g" "theme${theme}"/*.svg
		fi
	done
done
